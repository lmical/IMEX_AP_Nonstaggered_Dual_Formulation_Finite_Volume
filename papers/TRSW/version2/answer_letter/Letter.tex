...
\begin{enumerate}
	...
	\medskip
	{\sc\underline{Reply}}: We thank the referee for pointing out this ambiguity. The referee is entirely correct: the scaling of the parameter ratios to $1$ does not actively enforce, per se, the geostrophic balance itself, but rather it represents the choice of a specific physical regime that allows us to perform the formal asymptotic analysis and derive the balanced TQG equations as $\varepsilon \to 0$.
	
	We have clarified this in the section.
	...
	\medskip
	{\sc\underline{Reply}}: We thank the referee for this helpful suggestion. We have updated Equation~(2.8) to explicitly include the relation $\mathrm{Ro} / \mathrm{Bu} = \varepsilon / \nu$.
    ...
    \medskip
    {\sc\underline{Reply}}: The asymptotic analysis presented here is not new, and follows the one from~\cite{Warneford_2013}. We have added a clarification on this.
    
    {\large \textcolor{magenta}{Please check. But it should be correct.}}
...
\begin{enumerate}
	...
	\medskip
	{\sc\underline{Reply}}: We have updated the proof as required by the referee.
	...
	\medskip
	{\sc\underline{Reply}}: We thank the referee for this comment. We do not claim here a full inf-sup stability result for the fully discrete scheme. 
	%
	However, we added a proof in the manuscript in which we establish well-posedness of the linear systems (and hence of the resulting numerical methods), see Theorem 3.2 and Remark 3.3. 
	
	Concerning the choice of the gradients and of the other differential operators, we have used central differences and this choice is explicitly detailed in Section 3.3.2, where we have added also our definition of discrete divergence.
	
	% Classical collocated-grid issue.
	% With centered differences, the checkerboard pressure mode p_i=(-1)^i gives
	% D_x p_i = (p_{i+1}-p_{i-1})/(2 dx)=0, even though p_i is not constant.
	% Thus this pressure mode does not enter the discrete velocity equation
	%
	%     u_t + D_x p = ...
	%
	% since D_x p = 0. Analogously, checkerboard velocity modes may satisfy
	%
	%     D_x u = 0
	%
	% at the discrete level. Therefore, the discrete gradient-divergence coupling
	% may fail to control grid-scale oscillations, leading to pressure-velocity
	% decoupling. In our scheme, the semi-implicit update
	% is formulated as a fully coupled linear system, whose well-posedness is
	% proved in Theorem 3.2; no such effects were observed in the tests.
	
	Concerning checkerboard modes, these have not been observed in the numerical tests considered in the manuscript, nor in~\cite{CKKM}. We agree that such modes may be problematic in collocated discretizations when the discrete gradient and divergence operators do not provide a sufficiently coupled discretization. In the present scheme, however, the SI update is formulated through a fully (locally) coupled linear system, whose well-posedness is established in Theorem 3.2.
	%
	We have added a Remark on this after the newly introduced Theorem 3.2.
...
\begin{enumerate}
	...
	\medskip
	{\sc\underline{Reply}}: 
	The choice of the switching function is aligned with the system’s asymptotics, as it is decreasing and smoothly going from 1 to 0 as $\varepsilon \Rightarrow 0$... (ACTUALLY NO)
	
	
	
	%
   {\large \textcolor{magenta}{Easy to answer. To be written, but I get what the referee is saying here. Our choice, $\varphi(\eps)=\exp(-2000\eps^6)$, may be numerically ok. But the problem is that for $\eps=1$, $\varphi(\eps)=\exp(-2000)$, which is very small indeed but not exactly $0$. Indeed a bit of "crafting" is needed, but this makes it look more artificial than needed. Nan, look at my paper. My choice provides a smooth transition from 0 to 1 in $\varepsilon\in[0,1]$.
   If re-running all simulations is not feasible because it takes too much time, we can indeed justify the current choice.
   }}
