...
\begin{enumerate}
	...
	\medskip
	{\sc\underline{Reply}}: We thank the referee for this comment. We do not claim here a full inf-sup stability result for the fully discrete scheme. The implicit step leads to a linear elliptic problem, which is discretized using centered finite differences. The same type of centered discretization is used for the gradient, divergence, and Laplacian operators entering the semi-implicit update, so that the elliptic solve is based on a consistent discrete grad-div coupling.
	
	We have added a remark in Section~3.3.2 to clarify the structure of the implicit solve, the choice of the centered finite-difference operators, and the fact that no checkerboard modes or grid-decoupling effects were observed in the numerical experiments.
