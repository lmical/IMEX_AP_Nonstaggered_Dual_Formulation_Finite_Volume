...
\begin{frame}
	\frametitle{Governing equations}
		...
		\begin{equation*}
			\label{sys}
			\frac{\partial}{\partial t}\uvec{u}(\uvec{x},t)+div_{\uvec{x}} \boldsymbol{f}(\uvec{u}(\uvec{x},t))=\boldsymbol{0}, \quad (\uvec{x},t) \in \Omega\times \mathbb{R}^+_0,
		\end{equation*}
		\visible<2->{
			\begin{itemize}
				...
				\visible<4->{\item $\boldsymbol{f}:\mathcal{U} \rightarrow \mathbb{R}^{Q \times D}$ flux}
				\visible<5->{\item $\boldsymbol{f}(\uvec{u})=(\boldsymbol{f}_1,\boldsymbol{f}_2,\dots,\boldsymbol{f}_D) \in \mathbb{R}^{Q \times D}$ with $div_{\uvec{x}} \boldsymbol{f}(\uvec{u})=\sum\limits_{d=1}^D \frac{\partial \boldsymbol{f}_{d}}{\partial x_d} (\uvec{u}) \in \mathbb{R}^{Q}$} 
				\visible<6->{\item $\sum\limits_{d=1}^D \omega_d \frac{\partial \boldsymbol{f}_{d}}{\partial \uvec{u}} (\uvec{u})$ real diagonalizable $\forall \uvec{u}\in \mathcal{U}$, $\forall \uvec{\omega}\in \mathbb{R}^D$}
...
\begin{frame}[label=BalanceLawsSomeExamples]
	...
	$$\frac{\partial}{\partial t}\uvec{u}+div_{\uvec{x}} \boldsymbol{f}(\uvec{u})=\boldsymbol{0},$$
...
\begin{frame}
	\frametitle{A structural property: Conservation}
		\begin{equation*}
			\frac{\partial}{\partial t}\uvec{u}+ \frac{\partial}{\partial x} \boldsymbol{f}(\uvec{u})=\boldsymbol{0}, \quad (x,t) \in \Omega\times \mathbb{R}^+_0
		\end{equation*}
		$$\Downarrow$$
$$\int_\Omega \left( \frac{\partial}{\partial t}\uvec{u}+ \frac{\partial}{\partial x} \boldsymbol{f}(\uvec{u}) \right) d \uvec{x} =\boldsymbol{0}$$
		$$\Downarrow$$
$$\frac{d}{d t} \int_\Omega \uvec{u} d \uvec{x} = - \left[\boldsymbol{f}(\uvec{u})\right]_{x=x_L}^{x=x_R}   =\boldsymbol{0}$$
\end{frame}
...
\begin{frame}
	\frametitle{Actual issue with nonconservative products}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item (Easily) apply it to shock--detection
	\end{itemize}
	}
