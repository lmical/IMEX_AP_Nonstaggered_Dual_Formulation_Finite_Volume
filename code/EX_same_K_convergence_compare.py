...
teststocompare.append([name_base_folder,nametest,space_reconstruction,time_scheme,K,str(0.5)  , CFL]) 
teststocompare.append([name_base_folder,nametest,space_reconstruction,time_scheme,K,str(0.1)  , CFL]) 
...
ax_rou  = fig.add_subplot(142)
ax_rov  = fig.add_subplot(143)
...
ax_rou.clear()
ax_rou.set_title(r"$rou$")
ax_rou.set_xlabel('N')
ax_rou.set_ylabel('Error')

ax_rov.clear()
ax_rov.set_title(r"$rov$")
ax_rov.set_xlabel('N')
ax_rov.set_ylabel('Error')
...
for test in teststocompare:
    ...
    if os.path.isdir(foldName):  #CONDITION: Is it a folder? If yes go on
        ...
        if count>1: #If yes go on, MAKE THE CONVERGENCE ANALYSIS
            ...
            print(nametest, "with epsilon", iepsilon)
            ...
            print("   N       L1 error rho      Order rho      L1 Error rhou     Order rhou      L1 Error rhov     Order rhov         L1 Error E        Order E")
            fid.write("   N   & L1 error rho  &  Order rho & L1 Error rhou  &  Order rhou & L1 Error rhov  &  Order rhov & L1 Error E  &  Order E\\ \n")  
            ...
            ax_rou.loglog(numberofelements,errors[:,1] ,label="space"+str(ispace_reconstruction)+" time"+str(itime_scheme)+" eps"+str(iepsilon)+" K"+str(iK_coefficient))
            ax_rov.loglog(numberofelements,errors[:,2] ,label="space"+str(ispace_reconstruction)+" time"+str(itime_scheme)+" eps"+str(iepsilon)+" K"+str(iK_coefficient))
            ax_E.loglog(numberofelements,errors[:,3] ,label="space"+str(ispace_reconstruction)+" time"+str(itime_scheme)+" eps"+str(iepsilon)+" K"+str(iK_coefficient))
...
ax_rou.loglog(numberofelements,numberofelements**(-1),"--",linewidth=1,label="order 1")
ax_rou.loglog(numberofelements,1e1*numberofelements**(-2),"--",linewidth=1,label="order 2")

ax_rov.loglog(numberofelements,numberofelements**(-1),"--",linewidth=1,label="order 1")
ax_rov.loglog(numberofelements,1e1*numberofelements**(-2),"--",linewidth=1,label="order 2")

ax_E.loglog(numberofelements,numberofelements**(-1),"--",linewidth=1,label="order 1")
ax_E.loglog(numberofelements,1e1*numberofelements**(-2),"--",linewidth=1,label="order 2")
