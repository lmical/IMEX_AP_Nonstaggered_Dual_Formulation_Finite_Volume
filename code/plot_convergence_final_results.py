...
post_processing_vector=[5] 
setting_vector=["IMEX"] #"IMEX","EX"
space_reconstruction_vector=[23] #[20,21,22,23,24] #[23]
order_time_vector=[-22] #[2,3,4,5] #[2,3]
CFL=0.25
...
# refabscissae=np.array([25.0,50.0,100.0,200.0,400.0])

####################################################
name_post_processing = {0:"NOPP",1:"PP",5:"PP"}
colors_setting = {"IMEX":"#1f77b4","EX":"#ff7f0e"}
colors_eps={1e-0:"#1f77b4", 1e-1:"#ff7f0e", 1e-2:"#2ca02c", 1e-3:"#d62728", 1e-4:"#9467bd", 1e-5:"#8c564b", 1e-6:"#e377c2", 1e-7:"#7f7f7f", 1e-8:"#bcbd22", 1e-9:"#17becf"}
linestyles_order_time        = {2:"-",3:"--",4:"-.",5:":",-22:"-"}
            ...
            for post_processing in post_processing_vector:
                for space_reconstruction in space_reconstruction_vector:
                    for order_time in order_time_vector:
                        foldName="FINAL_RESULTS_time_step_"+variables+"/"+nametest+"/eps"+str(epsilon)+"/"+scheme+"/PP"+str(post_processing)+"/"+setting+"/space_reconstruction_"+str(space_reconstruction)+"/DeC"+str(order_time)+"/CFL"+str(CFL)
                        if os.path.isdir(foldName):  #CONDITION: Is it a folder? If yes go on
                            ...
                            if count>1: #If yes go on, MAKE THE CONVERGENCE ANALYSIS
                                ...
                                if setting=="EX":
                                    ...
                                    quit()
                                pl.loglog(numberofcells_X,errors[:,0],markersize=ms,marker=markers_space_reconstruction[space_reconstruction],linestyle=linestyles_order_time[order_time], linewidth=1.5,label="Mach="+str(epsilon),color=colors_eps[epsilon])


pl.loglog(numberofcells_X,3e-1*numberofcells_X**(-1),":",linewidth=2,label="order 2",color="0.70")
pl.loglog(numberofcells_X,5e-0*numberofcells_X**(-2),":",linewidth=2,label="order 2",color="0.50")
# pl.loglog(refabscissae,10e-0*refabscissae**(-3),":",linewidth=2,label="order 3",color="0.30")
...
pl.grid()
pl.savefig("Final_Results_"+nametest+"_"+variables+"_"+error_file_start+"convergence_Euler_2D.pdf", format="pdf", bbox_inches="tight")
