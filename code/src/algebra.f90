...
CONTAINS
  !*Inversion of a matrix -> Mat1=Inverse(Mat) I don't need the extra details.
  !*I just need to know that through this module you have a command to invert a matrix
  !*Inverse
  !------------------------------------------
  !!--  Inversion d'une matrice carree par LU et pivot partiel par ligne
  !!--  Ref : Numerical recipes in C
  !------------------------------------------
  !*Inversion of a matrix -> Mat1=Inverse(Mat)
  FUNCTION Inverse( Mat)  RESULT (Mat1)
    CHARACTER(LEN = *), PARAMETER :: mod_name = "InverseLu" !*Name
    REAL, DIMENSION(:, : ), INTENT(IN) :: Mat !*Matrix in input
    REAL, DIMENSION(SIZE(Mat, dim = 1), SIZE( Mat, dim = 1)) :: Mat1, mat2 !*Matrices of the same dimension of Mat matrix in input
    ...
    IF (.NOT. initialise) THEN !*initialise=false as default -> .not.initialise is true
       optim1_plain = .FALSE.
       optim2_plain = .TRUE.
    ...
    END IF
    !*All the logical are false

    Nsize = SIZE(Mat, dim=1)
    mat2  = Mat

    CALL ludcmp(mat2, Nsize, indx, d) !-- indx OUT !*Down here
