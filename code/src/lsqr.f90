...
!  * Jacob Williams : 8 Nov 2019 : created module
   ...
   type,public,extends(lsqr_solver) :: lsqr_solver_ez
      ...
      real,dimension(:),allocatable :: a    !! sparse `A` matrix

      real :: atol    = zero  !! relative error in definition of `A`
      real :: btol    = zero  !! relative error in definition of `b`
      real :: conlim  = zero  !! An upper limit on `cond(Abar)`, the apparent
                                  !! condition number of the matrix `Abar`.
      integer  :: itnlim  = 100000   !! max iterations
      integer  :: nout    = 0     !! output unit for printing

      ! used in aprod_ez:
      real,dimension(:),allocatable :: Ax   !! `A*x` (dimension m)
      real,dimension(:),allocatable :: Aty  !! `A(transpose)*y` (dimension n)

      real,dimension(:),allocatable :: v  !! workspace array (dimension n)
      real,dimension(:),allocatable :: w  !! workspace array (dimension n)
...
!  Constructor for [[lsqr_solver_ez]].
   ...
   real,dimension(:),intent(in)  :: a       !! nonzero elements of `A`
   real,intent(in),optional      :: atol    !! relative error in definition of `A`
   real,intent(in),optional      :: btol    !! relative error in definition of `b`
   real,intent(in),optional      :: conlim  !! An upper limit on `cond(Abar)`, the apparent
                                                !! condition number of the matrix `Abar`.
...
!  The internal `aprod` function for the [[lsqr_solver_ez]] class.
    ...
    integer,intent(in) :: m     !! number of rows in `A` matrix
    integer,intent(in) :: n     !! number of columns in `A` matrix
    real,dimension(:),intent(inout) :: x  !! [n]
    real,dimension(:),intent(inout) :: y  !! [m]
...
!  Wrapper for [[lsqr]] for the easy version of the class.
   ...
   class(lsqr_solver_ez),intent(inout)   :: me
   real,dimension(me%m),intent(in)   :: b
   real,intent(in)                   :: damp
   real,dimension(me%n),intent(out)  :: x       !! the computed solution `x`.
   integer,intent(out)                   :: istop   !! exit code (see [[lsqr]]).
   real,dimension(me%n),intent(out),optional :: se
   integer,intent(out) ,optional         :: itn
   real,intent(out),optional         :: anorm
   real,intent(out),optional         :: acond
   real,intent(out),optional         :: rnorm
   real,intent(out),optional         :: arnorm
   real,intent(out),optional         :: xnorm

   real,dimension(:),allocatable :: u  !! copy of `b` for call to [[lsqr]]
   real,dimension(:),allocatable :: se_
   logical  :: wantse   !! if `se` is to be returned
   integer  :: itn_
   real :: anorm_,acond_,rnorm_,arnorm_,xnorm_
...
!      if the computation is performed in higher precision.
   ...
   real,intent(in)   :: damp       !! The damping parameter for problem 3 above.
   ...
   logical,intent(in)    :: wantse     !! A logical variable to say if the array `se(*)`
                                       ...
                                       !! touched.
   real,intent(inout):: u(m)       !! The rhs vector `b`.  Beware that `u` is
                                       !! over-written by LSQR.
   real,intent(inout):: v(n)       !! workspace
   real,intent(inout):: w(n)       !! workspace
   real,intent(out)  :: x(n)       !! Returns the computed solution `x`.
   real,dimension(*),intent(out) :: se   !! If `wantse` is true, the dimension of `se` must be
                                             ...
                                             !! of any length.
   real,intent(in)   :: atol       !! An estimate of the relative error in the data
                                       !! defining the matrix `A`.  For example,
                                       !! if `A` is accurate to about 6 digits, set
                                       !! `atol = 1.0e-6`.
   real,intent(in)   :: btol       !! An estimate of the relative error in the data
                                       !! defining the rhs vector `b`.  For example,
                                       !! if `b` is accurate to about 6 digits, set
                                       !! `btol = 1.0e-6`.
   real,intent(in)              :: conlim   !! An upper limit on `cond(Abar)`, the apparent
                                                !! condition number of the matrix `Abar`.
   ...
   real,intent(out)  :: anorm      !! An estimate of the Frobenius norm of `Abar`.
   ...
   real,intent(out)  :: acond      !! An estimate of `cond(Abar)`, the condition
                                       !! number of `Abar`.  A very high value of `acond`
                                       !! may again indicate an error in `aprod`.
   real,intent(out)  :: rnorm      !! An estimate of the final value of `norm(rbar)`,
                                       !! the function being minimized (see notation
                                       !! above).  This will be small if `A*x = b` has
                                       !! a solution.
   real,intent(out)  :: arnorm     !! An estimate of the final value of
                                       !! `norm( Abar(transpose)*rbar )`, the norm of
                                       !! the residual for the usual normal equations.
                                       !! This should be small in all cases.  (arnorm
                                       !! will often be smaller than the true value
                                       !! computed from the output vector `x`.)
   real,intent(out)  :: xnorm      !! An estimate of the norm of the final
                                       !! solution vector `x`.

   logical :: damped
   integer :: i, maxdx, nconv, nstop
   real :: alfopt, alpha, beta, bnorm, &
               cs, cs1, cs2, ctol, &
   ...
   if (nout /= 0) then
      ...
      write(nout, '(1P,A,5X,A,E8.1,A)'   )        exit, '       =', dxmax/(xnorm + 1.0e-20), '*xnorm'
...
!   tol defined via power.
   ...
   integer,intent(out)  :: inform  !! Error indicator.
                                   ...
                                   !! consistent.
                                   !! inform = 1 otherwise.
   real,intent(in)  :: eps     !! The machine precision.
   real             :: v(n)
   real             :: w(m)
   real             :: x(n)
   real             :: y(m)

   real,parameter :: power = 0.5   !! eps**power is used as the tolerance for judging
   ...
   integer :: i, j
   real :: alfa, beta, t, test1, test2, test3, tol
...
!   Stanford University.
   ...
   real,intent(in)  :: anorm    !! An estimate of norm(A) or
                                    !! norm( A, delta*I ) if delta > 0.
                                    !! Normally this will be available
                                    !! from LSQR or CRAIG.
   real,intent(in)  :: damp     !! Possibly defines a damped problem.
   real,intent(in)  :: eps      !! Machine precision.
   real,intent(out) :: test1    !! These are dimensionless quantities
   ...
   real,intent(out) :: test2    !! These are dimensionless quantities
                                    !! that should be "small" if x does
                                    !! seem to solve one of the systems.
                                    !! "small" means less than
                                    !! tol = eps**power, where power is
                                    !! defined as a parameter below.
   real,intent(out) :: test3    !! These are dimensionless quantities
                                    !! that should be "small" if x does
                                    !! seem to solve one of the systems.
                                    !! "small" means less than
                                    !! tol = eps**power, where power is
                                    !! defined as a parameter below.
   real,intent(in)  :: b(m)     !! The right-hand side of Ax = b etc.
   real,intent(out) :: u(m)     !! On exit, u = r (where r = b - Ax).
   real,intent(out) :: v(n)     !! On exit, v = A'r.
   real,intent(out) :: w(n)     !! On exit, w = A'r - damp^2 x.
   real,intent(in)  :: x(n)     !! The given estimate of a solution.

   real,parameter :: power = 0.5

   integer :: j
   real :: bnorm, dampsq, rho1, rho2, sigma1, sigma2, &
