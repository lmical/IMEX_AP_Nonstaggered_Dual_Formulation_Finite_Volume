...
def plotting(folder_name, what_plot='all'):
    ...
    if what_plot == 'conservative' or what_plot == 'all':
        ...
        if ax_LS:
            cont_Ma = ax_LS.contourf(X, Y, np.zeros_like(X), cmap='viridis')
            cbar_Ma = fig.colorbar(cont_Ma, ax=ax_LS)
    ...
    def plot_one_step(i):
        ...
        if what_plot == 'all':
            if ax_LS:
                cont_LS = ax_LS.contourf(X, Y, LEVELSET, cmap='viridis')
                if cbar_Ma:
                    cbar_Ma.remove()
                cbar_Ma = fig.colorbar(cont_LS, ax=ax_LS)
                cbar_Ma.set_label(r'$Level set$')
                ax_LS.set_title(r'$Level set$')
                ax_LS.set_xlabel('X')
                ax_LS.set_ylabel('Y')
