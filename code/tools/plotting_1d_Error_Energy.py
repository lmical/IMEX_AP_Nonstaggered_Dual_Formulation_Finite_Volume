...
def plotting(folder_name, what_plot='all'):
    ...
    files_solution = folder_name + 'WC_Err_SOLUTION_*.dat'
    ...
    def plot_one_step(i):
        ...
        # max_to_plot=5.6992424385022642E-003
        max_energy_error=np.max(ENERGY)
        avg_energy_error=np.average(ENERGY)
        KKK=1.0

        ax_ro.semilogy(z, ENERGY)
        # ax_ro.semilogy(z, max_to_plot*np.ones((len(z))), label="maxmax")
        ax_ro.semilogy(z, avg_energy_error*np.ones((len(z))), label="avg")
        ax_ro.semilogy(z, KKK*avg_energy_error*np.ones((len(z))), label="K*avg")
        # ax_ro.semilogy(z, max_to_plot*np.ones((len(z)))/len(z), label="maxmax/N")
        # ax_ro.semilogy(z, max_to_plot*np.ones((len(z)))/len(z)**2, label="maxmax/N**2")
        ax_ro.set_title(r"$\rhou$")
        ...
        if what_plot=='conservative' or what_plot=='all':
            ...
            ax_E.clear()
            ax_E.semilogy(z, ENERGY)
            ax_E.semilogy(z, max_to_plot*np.ones((len(z))), label="maxmax")
            ax_E.semilogy(z, avg_energy_error*np.ones((len(z))), label="avg")
            ax_E.semilogy(z, max_to_plot*np.ones((len(z)))/len(z), label="maxmax/N")
            ax_E.semilogy(z, max_to_plot*np.ones((len(z)))/len(z)**2, label="maxmax/N**2")
            ax_E.set_title(r'$E$')
