...
if os.path.isdir(testfolder):  #CONDITION: Is it a folder? If yes go on
    ...
    if count>1: #If yes go on, MAKE THE CONVERGENCE ANALYSIS
        ...
        pl.grid()
        pl.loglog(numberofelements,errors[:,1],"-+",linewidth=1.5,label='Error u')
        pl.loglog(numberofelements,errors[:,2],"-o",linewidth=1.5,label='Error v')
        pl.loglog(numberofelements,errors[:,3],"-o",linewidth=1.5,label='Error p')
