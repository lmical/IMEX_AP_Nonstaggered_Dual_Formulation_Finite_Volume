...
def plotting(folder_name, what_plot='all'):
    ...
    if variable=="X":
        ...
        ENERGY   = ENERGY[0,:]  
        LEVELSET = LEVELSET[0,:]  
        for indi in range(len(LEVELSET)):
            # print(indi,LEVELSET[indi])
            if LEVELSET[indi]*LEVELSET[indi-1]<0:   
                P[indi]=(P[indi-1]+P[indi+1])/2                
        PHI = PHI[0,:]            
    elif variable=="Y":
        z=y
        RO  = RO[:,0] 
    ...
    if what_plot=='all':
        ...
        ax_LS.clear()
        ax_LS.plot(z, LEVELSET, " .", markersize=2.0, color="magenta")
