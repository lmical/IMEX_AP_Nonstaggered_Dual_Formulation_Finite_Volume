...
J=3;       %number of cell averages (odd)
K=(J+1)/2; %number of stencils and stencil length
...
for m=1:K
    ...
    for j=m:m+K-1
        for l=m:m+K-1
            fprintf("+(%.17f*Q(%d)*Q(%d))",double(beta(m,l,j)),j-K,l-K)
        end
    end
    fprintf("\n")
end
...
epsilon = 1e-8;  % Small regularization parameter
...
for x_quad = x_quads
    ...
    % Apply regularization to stabilize the inversion
    rhs=subs(hoCoefficients,y,x_quad);

    % First option: Regularized matrix inversion (Tikhonov regularization)
    %Amat = inv(double(c_quad * c_quad.') + epsilon * eye(size(c_quad, 1)));

    % Second option: Use SVD to handle potential near-singularities
    [U, S, V] = svd(double(c_quad * c_quad.'));
    S_inv = diag(1./diag(S));  
    S_inv(S < 1e-10) = 0;      % Threshold small singular values
    Amat = V * S_inv * U';

    d=simplify(Amat*(double(c_quad)*rhs.'));
    ...
    % Compute residual error to check stability
