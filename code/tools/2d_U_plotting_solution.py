...
def plotting(folder_name, what_plot='all'):
    ...
    cont_ro = ax_ro.contourf(X, Y, np.zeros_like(X), cmap='viridis')
    ...
    def plot_one_step(i):
        ...
        # # Plotting
        # cont_ro = ax_ro.contourf(X, Y, RO, cmap='coolwarm')
        # cont_ro.set_clim(0.4, 1.1)
        # cbar_ro.set_ticks(np.linspace(0.4, 1.1, num=8))
        # if cbar_ro:
        #     cbar_ro.remove()
        # cbar_ro = fig.colorbar(cont_ro, ax=ax_ro)
        # cbar_ro.set_label(r"$\rho$")
        ...
        vmin = np.min(RO)
        vmax = np.max(RO)
        cont_ro = ax_ro.contourf(X, Y, RO, cmap='jet') #jet
        # cont_ro.set_clim(0.4, 1.1)
        # cbar_ro.set_ticks(np.linspace(0.4, 1.1, num=8))
        if cbar_ro:
            cbar_ro.remove()
