...
# KKK=0.1 #0.015 #0.015 #0.015 #1.0 #0.015


parser = OptionParser()
parser.add_option("-d", "--dir",    action="store", type="string", dest="d", default="tsunami")
parser.add_option("-k", "--KKK", action="store", type="float", dest="KKK", default=1.0, help="KKK value")
...
def plotting(folder_name, what_plot='all'):
    ...
    def plot_one_step(i):
        ...
        avg_ro_error=np.average(RO)

        # Surface plots
        ax_ro.clear()
        ax_ro.semilogy(z, RO)
        ax_ro.semilogy(z, KKK*avg_ro_error*np.ones((len(z))), label="avg")
        ...
        if what_plot=='conservative' or what_plot=='all':
            if variable=="X":
                avg_qx_error=np.average(ROU)
                ax_qx.clear()
                ax_qx.semilogy(z, ROU)
                ax_qx.semilogy(z, KKK*avg_qx_error*np.ones((len(z))), label="avg")
            ...
            # max_to_plot=1.5025602056174685E-002
            ...
            ax_E.semilogy(z, KKK*avg_energy_error*np.ones((len(z))), label="avg")
