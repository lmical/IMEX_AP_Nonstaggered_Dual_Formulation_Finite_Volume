...
def plotting(folder_name, what_plot='all'):
    ...
    def plot_one_step(i):
        ...
        avgROU = np.average(ROU)
        avgROV = np.average(ROV)

        # Create a binary mask (1 for white, 0 for black)
        binary_mask = np.where( (ROU > KKK * avgROU) | (ROV > KKK * avgROV) , 0, 1)

        # Plot binary image (black and white)
        cont_ro = ax_ro.imshow(binary_mask, cmap='gray', extent=[X.min(), X.max(), Y.min(), Y.max()], origin='lower')

        if cbar_ro:
            cbar_ro.remove()
        cbar_ro = fig.colorbar(cont_ro, ax=ax_ro)
        cbar_ro.set_label("shock inditicator")
        ax_ro.set_title("shock inditicator")
        ax_ro.set_xlabel('X')
        ax_ro.set_ylabel('Y')
