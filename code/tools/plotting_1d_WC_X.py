...
parser.add_option("-v", "--value", action="store", type="float", dest="value", default=1.0, help="Mach")
...
def plotting(folder_name, what_plot='all'):
    ...
    files_solution = folder_name + 'WC_SOLUTION_*.dat'
    ...
    import_solution = [file for file in os.listdir(folder_name) if file.startswith('WC_SOLUTION_OCT')]
    ...
    def plot_one_step(i):
        ...
        ENERGY=P/(gmm-1)+0.5*eps**2*RO*(U**2+V**2)
