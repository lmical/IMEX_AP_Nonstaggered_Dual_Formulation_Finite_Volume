...
#CFL=0.1 works up to order 13
#100 elements
#######################
nametest="RP5"
file="SOLUTION_OCT_0000000.0120000.dat"
CFL_test=0.01 #0.01 #0.05 #0.5 #0.95 #OK
...
x1, x2, y1, y2 = 0.06, 0.13, 0.97, 1.005  # Specify the limits of the zoomed-in area
...
# Set the limits for the zoomed-in region
x1, x2, y1, y2 = 0.36, 0.53, 0.54, 0.635  # Specify the limits of the zoomed-in area
...
# Set the limits for the zoomed-in region
x1, x2, y1, y2 = 0.84, 0.86, 0.9, 3.  # Specify the limits of the zoomed-in area
...
# Set the limits for the zoomed-in region
x1, x2, y1, y2 = 0.796, 0.85, 5.3, 7.  # Specify the limits of the zoomed-in area
...
# Create an inset axes object with specific dimensions and position it outside the main plot area
axins5 = inset_axes(ax, width="80%", height="80%", loc='upper right', bbox_to_anchor=(0.28, -0.45, 0.4, 0.4), bbox_transform=fig.transFigure)
# Set the limits for the zoomed-in region
x1, x2, y1, y2 = 0.75, 0.805, 0.4, 2.0  # Specify the limits of the zoomed-in area
axins5.set_xlim(x1, x2)
axins5.set_ylim(y1, y2)
axins5.set_title("E",fontsize=fs)
# Add a rectangle to highlight the zoomed area in the main plot
...
if os.path.isdir(solfoldName):  #CONDITION: Is it a folder? If yes go on
    ...
    if os.path.isfile(namefile): #If the file exists, read it
        ...
        axins5.plot(x_ref,ro_ref,linestyle="-",linewidth=lwex,label=solution_type,color="k")
        ...
        if os.path.isfile(namefile): #If the file exists, read it
            ...
            if os.path.isfile(foldName+"/"+ filename):
                ...
                if os.path.isfile(foldName+"/"+ file):
                    ...
                    axins5.plot(z,RO,marker=markers_space_reconstruction[space_reconstruction],linestyle=linestyles_space_reconstruction[space_reconstruction],linewidth=lw, markersize=ms,label=label_space_reconstruction[space_reconstruction]+" "+label_time_scheme[time_scheme],color=colors[space_reconstruction],alpha=0.7,zorder=3)
